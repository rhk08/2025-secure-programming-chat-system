...
class Server:
    def __init__(self, host="127.0.0.1", port=9000, introducers=None, introducer_mode=False):
        ...
        self.public_channel_id = "g123"  # Fixed public channel ID
        self.public_channel_members_local = set()  # Local members of public channel
        self.public_channel_version = 1  # Version counter for public channel updates
        # user_id -> wrapped_key for public channel
        self.public_channel_wrapped_keys = {}
    ...
    async def cleanup_client(self, username):
        # Remove from public channel if they were a member
        was_public_member = username in self.public_channel_members_local
        if was_public_member:
            self.public_channel_members_local.discard(username)
            self.public_channel_wrapped_keys.pop(username, None)
        ...
        # If they were a public channel member, broadcast updated channel state
        if was_public_member:
            await self.broadcast_public_channel_updated()

            continue

    async def broadcast_public_channel_updated(self):
        """Broadcast PUBLIC_CHANNEL_UPDATED message with current wrapped keys"""
        # Increment version
        self.public_channel_version += 1

        # Build wraps array for all known public channel members
        wraps = []
        for member_id in self.public_channel_members_local:
            # Get wrapped key for this member (placeholder - would be generated by key management)
            wrapped_key = self.public_channel_wrapped_keys.get(
                member_id, "placeholder_wrapped_key")
            wraps.append({
                "member_id": member_id,
                "wrapped_key": wrapped_key
            })

        # Create PUBLIC_CHANNEL_UPDATED message
        updated_message = deepcopy(self.JSON_base_template)
        updated_message["type"] = "PUBLIC_CHANNEL_UPDATED"
        updated_message["from"] = self.server_uuid
        updated_message["to"] = "*"
        updated_message["ts"] = time.time()
        updated_message["payload"] = {
            "version": self.public_channel_version,
            "wraps": wraps
        }
        updated_message["sig"] = codec.generate_payload_signature(
            updated_message,
            self.private_key
        )
        ...
        for server_id, link in self.servers.items():
            try:
                await link.websocket.send(json.dumps(updated_message))
                print(
                    f"[{self.server_uuid}] Sent PUBLIC_CHANNEL_UPDATED to server {server_id}")
            except Exception as e:
                print(
                    f"[{self.server_uuid}] Failed to send PUBLIC_CHANNEL_UPDATED to {server_id}: {e}")
    ...
    async def incoming_connection_handler(self, ws):
        ...
        try:
            async for msg in ws:
                ...
                # Handle PUBLIC_CHANNEL_ADD from other servers
                if msg_type == "PUBLIC_CHANNEL_ADD":
                    payload = frame.get("payload", {})
                    added_users = payload.get("add", [])
                    if_version = payload.get("if_version", 1)

                    # Verify server signature
                    server_id = frame.get("from")
                    if server_id in self.server_addrs:
                        _, _, pubkey = self.server_addrs[server_id]
                        pubkey_obj = codec.decode_public_key_base64url(pubkey)
                        codec.verify_payload_signature(frame, pubkey_obj)

                        # Update local public channel members if version matches
                        if if_version == self.public_channel_version:
                            for user_id in added_users:
                                if user_id not in self.public_channel_members_local:
                                    self.public_channel_members_local.add(
                                        user_id)
                                    print(
                                        f"[{self.server_uuid}] Added {user_id} to public channel")

                            # Increment version
                            self.public_channel_version += 1

                            # Forward to other servers
                            for other_server_id, link in self.servers.items():
                                if other_server_id != server_id:
                                    try:
                                        await link.websocket.send(json.dumps(frame))
                                        print(
                                            f"[{self.server_uuid}] Forwarded PUBLIC_CHANNEL_ADD to {other_server_id}")
                                    except Exception as e:
                                        print(
                                            f"[{self.server_uuid}] Failed to forward PUBLIC_CHANNEL_ADD to {other_server_id}: {e}")
                    continue

                # Handle PUBLIC_CHANNEL_UPDATED from other servers
                if msg_type == "PUBLIC_CHANNEL_UPDATED":
                    ...
                    if server_id in self.server_addrs:
                        ...
                        # Update wrapped keys for local members
                        for wrap in wraps:
                            member_id = wrap.get("member_id")
                            wrapped_key = wrap.get("wrapped_key")
                            if member_id in self.public_channel_members_local:
                                self.public_channel_wrapped_keys[member_id] = wrapped_key
                                print(
                                    f"[{self.server_uuid}] Updated wrapped key for {member_id}")

                        # Update version
                        self.public_channel_version = max(
                        ...
                        for other_server_id, link in self.servers.items():
                            if other_server_id != server_id:
                                try:
                                    await link.websocket.send(json.dumps(frame))
                                    print(
                                        f"[{self.server_uuid}] Forwarded PUBLIC_CHANNEL_UPDATED to {other_server_id}")
                                except Exception as e:
                                    print(
                                        f"[{self.server_uuid}] Failed to forward PUBLIC_CHANNEL_UPDATED to {other_server_id}: {e}")
                    continue

                # Handle PUBLIC_CHANNEL_KEY_SHARE from other servers
                if msg_type == "PUBLIC_CHANNEL_KEY_SHARE":
                    ...
                    if server_id in self.server_addrs:
                        ...
                        # Route each share to the correct member
                        for share in shares:
                            member_id = share.get("member")
                            wrapped_key = share.get(
                                "wrapped_public_channel_key")

                            if member_id in self.local_users:
                                # Deliver to local member
                                try:
                                    key_share_msg = deepcopy(
                                        self.JSON_base_template)
                                    key_share_msg["type"] = "PUBLIC_CHANNEL_KEY_SHARE"
                                    key_share_msg["from"] = self.server_uuid
                                    key_share_msg["to"] = member_id
                                    key_share_msg["ts"] = frame.get(
                                        "ts", time.time())
                                    key_share_msg["payload"] = {
                                        "wrapped_public_channel_key": wrapped_key,
                                        "creator_pub": creator_pub,
                                        "content_sig": content_sig
                                    }
                                    await self.local_users[member_id].websocket.send(json.dumps(key_share_msg))
                                    print(
                                        f"[{self.server_uuid}] Delivered key share to local member {member_id}")
                                except Exception as e:
                                    print(
                                        f"[{self.server_uuid}] Error delivering key share to {member_id}: {e}")
                                    await self.cleanup_client(member_id)
                            else:
                                # Route to remote server
                                member_server = self.user_locations.get(
                                    member_id)
                                if member_server and member_server in self.servers:
                                    try:
                                        await self.servers[member_server].websocket.send(json.dumps(frame))
                                        print(
                                            f"[{self.server_uuid}] Forwarded key share to {member_server} for {member_id}")
                                    except Exception as e:
                                        print(
                                            f"[{self.server_uuid}] Failed to forward key share to {member_server}: {e}")
                    continue
                ...
                if msg_type == "SERVER_PUBLIC_CHANNEL":
                    ...
                    # Forward to other servers for complete fan-out
                    for other_server_id, link in self.servers.items():
                        if other_server_id != frame.get("from"):
                            try:
                                await link.websocket.send(json.dumps(frame))
                                print(
                                    f"[{self.server_uuid}] Forwarded SERVER_PUBLIC_CHANNEL to {other_server_id}")
                            except Exception as e:
                                print(
                                    f"[{self.server_uuid}] Failed to forward SERVER_PUBLIC_CHANNEL to {other_server_id}: {e}")
                    continue
                ...
                if msg_type == "USER_HELLO":
                    ...
                    # Add user to local public channel members
                    ...
                    # PUBLIC_CHANNEL_ADD broadcast to all servers
                    public_channel_add_message = deepcopy(
                        self.JSON_base_template)
                    public_channel_add_message["type"] = "PUBLIC_CHANNEL_ADD"
                    public_channel_add_message["from"] = self.server_uuid
                    public_channel_add_message["to"] = "*"
                    public_channel_add_message["ts"] = time.time()
                    public_channel_add_message["payload"] = {
                        "add": [client_id],
                        "if_version": self.public_channel_version
                    }
                    public_channel_add_message["sig"] = codec.generate_payload_signature(
                        public_channel_add_message,
                        self.private_key
                    )

                    # Broadcast PUBLIC_CHANNEL_ADD to all connected servers
                    for server_id, link in self.servers.items():
                        try:
                            await link.websocket.send(json.dumps(public_channel_add_message))
                            print(
                                f"[{self.server_uuid}] Sent PUBLIC_CHANNEL_ADD to server {server_id}")
                        except Exception as e:
                            print(
                                f"[{self.server_uuid}] Failed to send PUBLIC_CHANNEL_ADD to {server_id}: {e}")

                    # Generate and broadcast PUBLIC_CHANNEL_UPDATED with wrapped keys
                    await self.broadcast_public_channel_updated()
                ...
                if msg_type == "MSG_PUBLIC_CHANNEL":
                    ...
                    for member_id in list(self.public_channel_members_local):
                        if member_id in self.local_users and member_id != sender:
                            try:
                                deliver_msg = deepcopy(self.JSON_base_template)
                    ...
                    # Forward to peer servers for fan-out to all known members
                    for server_id, link in self.servers.items():
                        try:
                            server_msg = deepcopy(self.JSON_base_template)
                            server_msg["type"] = "SERVER_PUBLIC_CHANNEL"
                            server_msg["from"] = self.server_uuid
                            server_msg["to"] = server_id
                            server_msg["ts"] = frame.get("ts", time.time())
                            server_msg["payload"] = {
                                "channel_id": self.public_channel_id,
                                "ciphertext": ciphertext,
                                "sender": sender,
